...
{
#if NDEBUG
	gfx.DrawSprite( 0,0,*jupiterBG,SpriteEffect::Copy{} );
#endif
...
void LevelEditor::WriteToFile()
{
	...
	for( const auto& item : entities[int( Entity::JupiterPlatform )] )
	{
		out += "JupiterFloor|";
		out += std::to_string( item.first.x ) + '|';
		out += std::to_string( item.first.y ) + '|';
	...
	for( const auto& item : entities[int( Entity::JupiterCrystal )] )
	{
		out += "JupiterCrystal|";
		out += std::to_string( item.first.x ) + '|';
		out += std::to_string( item.first.y ) + '\n';
	...
	for( const auto& item : entities[int( Entity::MarsTurret )] )
	{
		out += "MarsTurret|";
		out += std::to_string( item.first.x ) + '|';
		out += std::to_string( item.first.y ) + '|';
	...
	for( const auto& item : entities[int( Entity::MarsDiver )] )
	{
		out += "MarsDiver|";
		out += std::to_string( item.first.x ) + '|';
		out += std::to_string( item.first.y ) + '\n';
	}
	for( const auto& item : entities[int( Entity::GravityFlipper )] )
	{
		out += "GravityFlipper|";
		out += std::to_string( item.first.x ) + '|';
		out += std::to_string( item.first.y ) + '\n';
	}
	for( const auto& item : entities[int( Entity::GravitySlower )] )
	{
		out += "GravitySlower|";
		out += std::to_string( item.first.x ) + '|';
		out += std::to_string( item.first.y ) + '\n';
	}
	for( const auto& item : entities[int( Entity::GravityRotator )] )
	{
		out += "GravityRotator|";
		out += std::to_string( item.first.x ) + '|';
		out += std::to_string( item.first.y ) + '\n';
